 ...
 *     在对{@link ViewGroup＃getChildAt}的调用中使用的附加子视图的索引。
 *
 ...
 *     or construction.</li>
 *      以前用于显示特定适配器位置的数据的视图可能被放置在缓存中，以备日后重新使用，以便以后再次显示相同类型的数据。
 *      这可以通过跳过初始布局或建设来大幅度提高性能。
 *
 ...
 *     在布局过程中进入临时分离状态的子视图。 Scrap views可以重复使用而不会完全脱离父级RecyclerView，
 *     如果不需要重新绑定，或者如果视图被认为是脏的
 *
 ...
 * <p>
 * RecyclerView在“连接适配器”和“连接器布局管理器”之间引入了一个额外的抽象级别，以便能够在布局计算过程中检测批次中的数据集更改。
 * 这可以保存LayoutManager跟踪适配器更改以计算动画。
 * 它也有助于提高性能，因为所有的视图绑定同时发生，避免了不必要的绑定。
 *
 * For this reason, there are two types of <code>position</code> related methods in RecyclerView:
 * 为此，RecyclerView中有两种位置相关的方法
 * <ul>
 ...
 *     布局位置：项目在最新布局计算中的位置。这是从LayoutManager的角度来看的。
 *     适配器位置：适配器中物品的位置。这是从适配器的角度来看的。
 *
 * </ul>
 * <p>
 ...
 * <p>
 * 除了调度adapter.notify *事件和计算更新的布局之间的时间之外，这两个位置是相同的。
 *
 *
 ...
 * is seeing.
 *  返回或接收<code> * LayoutPosition * </ code>的方法使用最新布局计算的位置（例如{@link ViewHolder＃getLayoutPosition（）}，
 *  {@link #findViewHolderForLayoutPosition（int）}）。这些职位包括所有更改，直到最后的布局计算。
 *  您可以依靠这些职位来与目前在屏幕上看到的用户保持一致。例如，如果屏幕上有一个项目列表，并且用户要求输入第5个元素，
 *  则应该使用这些方法，因为它们将与用户看到的内容匹配。
 *
 * <p>
 ...
 * 另一组与位置有关的方法是以<code> * AdapterPosition * </ code>的形式。 （例如{@link ViewHolder＃getAdapterPosition（）}，
 * {@link #findViewHolderForAdapterPosition（int）}）当您需要使用最新的适配器位置（即使它们尚未反映到布局）时，您应该使用这些方法。
 * 例如，如果要在ViewHolder单击中访问适配器中的项目，则应该使用
 * {@link ViewHolder＃getAdapterPosition（）}。请注意，如果已调用{@link Adapter＃notifyDataSetChanged（）}并且尚未计算新布局，
 * 则这些方法可能无法计算适配器位置。出于这个原因，您应该仔细处理这些方法的{@link #NO_POSITION}或<code> null </ code>结果。
 * <p>
 * When writing a {@link LayoutManager} you almost always want to use layout positions whereas when
 * writing an {@link Adapter}, you probably want to use adapter positions.
 * 在编写LayoutManager时，您几乎总是要使用布局位置，而在编写{@link Adapter}时，您可能需要使用适配器位置。
 *
...
public class RecyclerView extends ViewGroup implements ScrollingView, NestedScrollingChild {
     ...
     * 此变量在dispatchLayout和/或scroll期间递增。在这些时间段内不应调用某些方法（例如，适配器数据更改）。
     * 这样做会很难找到错误，所以我们最好检查一下并抛出异常。
     *
     ...
     * 返回RecyclerView最近是否在计算布局
     * <p>
     * If this method returns true, it means that RecyclerView is in a lockdown state and any
     * attempt to update adapter contents will result in an exception because adapter contents
     * cannot be changed while RecyclerView is trying to compute the layout.
     * 如果这个方法返回true,它意味着RecyclerView处于一个锁定的状态,并且任何尝试更新适配器内容都将导致异常，
     * 因为在RecyclerView尝试计算布局时无法更改适配器内容。
     * <p>
     * It is very unlikely that your code will be running during this state as it is
     * called by the framework when a layout traversal happens or RecyclerView starts to scroll
     * in response to system events (touch, accessibility etc).
     * 在这种状态下运行代码是不太可能的，因为当布局遍历发生时，或者RecyclerView开始滚动以响应系统事件（触摸，辅助功能等）时，框架就会调用它。
     * <p>
     * This case may happen if you have some custom logic to change adapter contents in
     * response to a View callback (e.g. focus change callback) which might be triggered during a
     * layout calculation. In these cases, you should just postpone the change using a Handler or a
     * similar mechanism.
     * 如果您有一些自定义逻辑来更改适配器内容，以响应在布局计算期间可能触发的View回调（例如焦点更改回调），则可能会发生这种情况。
     ...
     * - process adapter updates 进程适配器更新
     * - decide which animation should run 决定应该运行哪个动画
     * - save information about current views 保存有关当前视图的信息
     * - If necessary, run predictive layout and save its information 如有必要，运行预测性布局并保存其信息
     */
     ...
     * waiting for VSYNC.
     * 在UI线程正在等待VSYNC的停机时间内，在遍历之后立即运行预取工作。
     */
    ...
    private class RecyclerViewDataObserver extends AdapterDataObserver {
        ...
        public void onChanged() {
            ...
            if (mAdapter.hasStableIds()) {
                // TODO: 2017/11/7 确定是否真正的改变
                // TODO Determine what actually changed.
                // 现在执行这个更重要，因为这个回调会禁用所有的动画，因为我们不能依靠位置。
     ...
     * 回收视图池允许在多个RecyclerView间共享view
     * <p>
     * If you want to recycle views across RecyclerViews, create an instance of RecycledViewPool
     * and use {@link RecyclerView#setRecycledViewPool(RecycledViewPool)}.
     * <p>
     * 如果你想跨RecyclerView复用view,创建一个RecyclerViewPoold的实例然后使用RecyclerView.setRecycledViewPool
     * 去设置你这个实例
     * RecyclerView automatically creates a pool for itself if you don't provide one.
     * 如果你没有提供的话RecyclerView将会为自己自动创建一个池
     */
    public static class RecycledViewPool {
        ...
        //设置每种viewType的view最大容量,超过的部分从列表的尾部移除
         ...
         * 回收池将会清理缓存如果仅有一个适配器关联并且新的适配器相比旧的适配器使用了一个不同的ViewHodler
         *
     ...
     * Recycler的责任是管理"报废"的和分离的item view以便再次使用
     *
     * <p>A "scrapped" view is a view that is still attached to its parent RecyclerView but
     * that has been marked for removal or reuse.</p>
     * 一个"报废"的view指的是仍然依附在父RecyclerView上但是已经被标记为移除或者重用的view
     *
     *
     ...
     *
     * Recycler的典型用法是通过LayoutManager去获取adapter数据集中给定位置或者条目id的view
     * 如何这个view重用被认为是"脏"适配器将会重新绑定view
     * 如果不是的话,这个view将会通过LayoutManger不做任何处理被快速重用
     * 干净的view如果不是主动调用View.requesteLayout()将不会通过LayoutManager重新测量然后被重新放置
     */
     ...
     * ViewCacheExtension是一个提供了额外视图缓存的帮助类,开发者可以自己控制视图的缓存
     * <p>
     * When {@link Recycler#getViewForPosition(int)} is called, Recycler checks attached scrap and
     * first level cache to find a matching View. If it cannot find a suitable View, Recycler will
     * call the {@link #getViewForPositionAndType(Recycler, int, int)} before checking
     * {@link RecycledViewPool}.
     * 当Recycler.getViewForPosition被回调时,Recycler会检查attached scrap和一级缓存来寻找匹配的view,
     * 如果没有找到合适的view,Recycler将会回调getViewForPositionAndType方法,如果还是没有的话,就去RecycledViewPool寻找
     * <p>
     * Note that, Recycler never sends Views to this method to be cached. It is developers
     * responsibility to decide whether they want to keep their Views in this custom cache or let
     * the default recycling policy handle it.
     *
     * 注意,Recycler不会将视图通过这个方法来缓存。开发人员有责任决定他们是否希望将自己的视图保留在此自定义缓存中，
     * 或者让默认的回收策略处理它。
     */
    ...
    public static abstract class Adapter<VH extends ViewHolder> {
         ...
         * may be altered.</p>val position = v.tag
         *
    ...
    public static abstract class LayoutManager {
         ...
         * 布局管理拥有多种严格的测量缓存去避免重复测量子View,如果一个给出的空间已经比之前测量的大
         *
         */
     ...
     * OnItemTouchListener允许应用程序在RecyclerView的视图层次结构级别拦截正在进行的触摸事件，然后将这些触摸事件视为RecyclerView自身的滚动行为。
     *
     ...
     * 对于希望在RecyclerView中实现各种形式的对项目视图进行手势操纵的应用程序，这是非常有用的。
     * OnItemTouchListeners可以拦截已经在进行的触摸交互，即使RecyclerView已经为了滚动的目的而已经处理了该手势流本身。
     *
     * @see SimpleOnItemTouchListener
     */
     ...
     * 具有空方法体和默认返回值的{@link RecyclerView.OnItemTouchListener}的实现。
     * <p>
     ...
     * 一个OnScrollListener可以被添加到一个RecyclerView中，以便在RecyclerView上发生滚动事件时接收消息。
     *
     * <p>
     ...
     * 一个Listener接口，可以连接到RecylcerView，以便在ViewHolder连接到RecyclerView或从RecyclerView分离时得到通知。
     */
     ...
     * 一个ViewHolder描述一个项目视图和关于它在RecyclerView中的位置的元数据。
     *
     ...
     * 平滑滚动的基础类,处理目标视图位置的基本跟踪,并提供出发编程滚动的方法
     *
    ...
    /**
     *
     * 包含有关当前RecyclerView状态的有用信息，如目标滚动位置或视图焦点。 状态对象还可以保留任意数据，由资源ID标识。
     * 通常情况下，RecyclerView组件将需要在彼此之间传递信息。
     * 为了在组件之间提供定义良好的数据总线，RecyclerView将相同的State对象传递给组件回调，这些组件可以使用它来交换数据。
    ...
    /**
     * This class defines the behavior of   if the developer wishes to handle it.
     * <p>
    ...
    public static abstract class OnFlingListener {
         ...
         * 注意，只有当关联的LayoutManager支持滚动并且不是由嵌套滚动首先处理的时候才会调用这个方法。
         *
     ...
     * 内部监听器管理动画完成后的项目。 这是项目在动画期间如何保留（不回收），但允许在之后回收。
     * 它取决于与ItemAnimator的合同，当动画完成任何项目动画时，在动画师的监听器上调用相应的dispatch * Finished（）方法。
     */
     ...
     * 一个回调接口，可以用来改变RecyclerView子项的绘制顺序。
     * <p>
