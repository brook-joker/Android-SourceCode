...
public class ConcurrentHashMap7<K, V> extends AbstractMap<K, V>
    ...
    static final class Segment<K, V> extends ReentrantLock implements Serializable {
        ...
        private void rehash(HashEntry<K, V> node) {
             ...
             * 因为我们使用的是二次幂扩展，所以每个节点的元素必须保持相同的索引，或者以两个偏移的幂移动。
             * are using power-of-two expansion, the elements from
             * each bin must either stay at same index, or move with a
             * power of two offset. We eliminate unnecessary node
             * 我们通过捕获旧节点可以重用的情况来消除不必要的节点创建，因为它们的下一个字段不会改变。
             * creation by catching cases where old nodes can be
             * reused because their next fields won't change.
             *  统计上，在默认阈值下，当表加倍时，只有大约六分之一的节点需要克隆。
             * Statistically, at the default threshold, only about
             * one-sixth of them need cloning when a table
             *  一旦它们不再被可能同时遍历表中的任何读者线程引用，它们替换的节点将是垃圾收集的。
             * doubles. The nodes they replace will be garbage
             * collectable as soon as they are no longer referenced by
             * any reader thread that may be in the midst of
             * 条目访问使用普通数组索引，因为它们后面是volatile表写入。
    ...
    public ConcurrentHashMap7(int initialCapacity,
        ...
        int sshift = 0;
        int ssize = 1;
        // ssize=“大于或等于concurrencyLevel的最小的2的N次方值” (如果concurrencyLevel=[9,15],那么ssize = 16)
