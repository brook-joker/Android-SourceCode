 ...
 * <p>
 *
 * Component类是与非菜单相关的Abstract Window Toolkit组件的抽象超类。类组件也可以直接扩展以创建轻量级组件。
 * 轻量级组件是与本机窗口无关的组件。相反，重量级组件与本机窗口相关联。可以使用isLightweight（）方法区分两种组件。
 *
 ...
 *
 * 轻量级和重型组件可以在单个组件层次结构中混合。然而，为了正确操作这种混合层次结构的组件，整个层次结构必须是有效的。
 * 当层次结构无效时，就像在更改组件的边界，或者向/从容器中添加/删除组件之后，
 * 整个层次结构必须通过在最上面的无效容器中调用的Container.validate（）方法进行验证层次结构。
 *
 * <h3>Serialization</h3>
                                           ...
                                           Serializable
{
     ...
     *
     * 组件的对等体,这个对等体实现了组件的行为. 这个对等体在组件添加到一个容器被设置
     *
     * @see #addNotify
     * @see #removeNotify
     */
     ...
     * operations.
     * 获得AWT组件树和布局操作的此组件的锁定对象(拥有线程同步监视器的对象)
     *
     ...
     * 判断当前这个组件是否是有效的,
     * 一个组件有效的条件是它有正确的大小和位置在它的父容器并且它的子组件也是有效的
     * 为了解决对等体的大小要求,组件在首次显示在屏幕之前都是无效的,
     * 等到父容器完全实现时,它的组件都将有效
     *
