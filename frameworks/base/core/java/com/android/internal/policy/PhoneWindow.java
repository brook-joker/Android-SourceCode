...
package frameworks.base.core.java.com.android.internal.policy;
...
public class PhoneWindow extends Window implements MenuBuilder.Callback {
    ...
    public void setContentView(int layoutResID) {
        ...
        // before this happens.
        // 根据layout的id加载一个布局,然后通过findViewById(R.id.content)加载出布局中id为content的FrameLayout
        // 赋值给mContentParent,并且将该view添加到mDecor(DecorView)中
        if (mContentParent == null) { //第一次是空
            //初始化decorView
            installDecor();
        } else if (!hasFeature(FEATURE_CONTENT_TRANSITIONS)) {
            //没有过度效果 并且不是第一次setContentView,那么要先移除内部的子View
        ...
        if (hasFeature(FEATURE_CONTENT_TRANSITIONS)) {
            ...
            //无过度效果 加载id为layoutResID的视图并且添加到mContentParent中
    ...
    @Override
    public final View getDgetDecorView() {
    ...
    protected DecorView generateDecor(int featureId) {
        ...
        if (mUseDecorContext) { //从activity的setContentView方法调用则为true
            ...
            if (applicationContext == null) {
                // 系统进程时没有Application的context，所以就用现有的context
                context = getContext();
            } else {
                // 应用会有application的Context
    ...
    protected ViewGroup generateLayout(DecorView decor) {
        ...
        // 这个layout布局文件在frameworks\base\core\res\res\layout\目录下 int layoutResource;
    ...
    private void installDecor() {
        ...
        //继承自FrameLayout 是窗口顶级视图,也就是Activity显示View的根View,包含一个titleView和contentView
        if (mDecor == null) { //首次为空
            //初始化DecorView（FrameLayout）
        ...
        if (mContentParent == null) { //第一次setContentView时为空
            ...
            //如果合适，设置UI的装饰部分以忽略fitsSystemWindows。
            mDecor.makeOptionalFitsSystemWindows();

            //判断是否存在id为decor_content_parent的view(screen_action_bar.xml这个里面有这个id)
